...
export class TranslationService {
  ...
  request(text) {
    ...
    return create_promise().catch(create_promise).catch(create_promise);
  }
  ...
  premium(text, minimumQuality) {
    const fetch_translation = (text, minimumQuality) => {
      return this.api.fetch(text).then(
        response => {
          if (response.quality >= minimumQuality) {
            return Promise.resolve(response.translation)
          } else {
            throw new QualityThresholdNotMet("insufficient quality");
          }
        });
    };
    return fetch_translation(text, minimumQuality)
      .catch(error => {
      ...
      .then(_ => fetch_translation(text, minimumQuality), err => { throw err });
  }
}
